...
def get_stop_words(add_stop=None, sub_stop=None):
    stop_words = ENGLISH_STOP_WORDS
    if add_stop:
        stop_words = stop_words.union(add_stop)
    if sub_stop:
        stop_words = stop_words.difference(sub_stop)
    return stop_words
...
def process_html_text(text):
    ...
    no_stop = [w for w in no_stop if len(w)>1 ]
    return no_stop
...
# return tokens after cleaning tex
def process_text(text):
    if text is None:
        print("None type text sent")
    ...
    stop_words = get_stop_words()
    no_stop = [w for w in no_punc if w not in stop_words]
    no_stop = [w for w in no_stop if len(w)>2 ]
    return no_stop

# return unique words, after cleaning text
def process_text_uw(text):
    no_stop = process_text(text)
